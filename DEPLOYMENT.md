...
alembic current                              # expect: 0006 (head)
```
...
- `alembic current`, `heads` and `history` create nothing. `alembic upgrade head`
  ...
  candidate tables `v2.company_candidate` and `v2.company_candidate_identifier`.
...
**`downgrade` past 0006 refuses to run while any candidate exists, past 0005 while `v2.processing_attempt` holds rows, past 0004 while
