...
- `backend-legacy-core-journey`: **verified locally only, not yet pushed.** Per each task's own constraints (no commit/push), this job has not run on GitHub Actions yet at its current, 10-file shape. All ten test files it now runs were verified in this session against a fresh, local, disposable PostgreSQL database (`createdb venturegps_legacy_ci_test`, dropped afterward) using the exact same `DATABASE_URL`/`OPENAI_API_KEY`/`TAVILY_API_KEY`/`CLERK_ISSUER` values the workflow sets — **142/142 individual tests passed** across all ten files, including `test_analysis_visibility.py`'s and `test_founder_workspace.py`'s cross-user privacy regression tests. See "Local verification results" below.
...
| `backend-legacy-core-journey` | Ten `app/tests/*.py` script-style test files covering the legacy backend's core user journey, AI request reliability, and secure analysis visibility — including Founder Workspace's own separate authorization path (see table below), against a fresh disposable PostgreSQL 16 service container | ~1–2 min combined (local validation, ten sequential steps) |
| `frontend` | `npm run lint` (ESLint), `npx tsc --noEmit` (TypeScript), `npm test` (26 suites), `npm run build` (Next.js production build) | ~1–2 min combined |

### `backend-legacy-core-journey`: which tests run, and which remain excluded

**Included (10 files, one CI step each, 142 individual tests):**
...
All ten files were run, in the exact order the workflow runs them, against a fresh local disposable PostgreSQL database (`createdb venturegps_legacy_ci_test`, dropped immediately afterward), with `DATABASE_URL`/`OPENAI_API_KEY`/`TAVILY_API_KEY`/`CLERK_ISSUER` set to the exact values the workflow sets (see the "Reproducing the checks locally" command block above) — the same commands and environment `backend-legacy-core-journey` runs, just outside GitHub Actions. This run is also the first time `test_analysis_visibility.py` and `test_founder_workspace.py` were verified against a genuinely fresh, CI-shaped database with the CI placeholder credentials, rather than this session's own local development database:
...
`test_analysis_visibility.py` and `test_founder_workspace.py` are both fully offline (no LLM/Tavily calls, no real Clerk instance contacted — each authenticated test signs its own local RS256 JWT and monkeypatches `app.auth`'s verification for its own duration) and both use distinctive, self-cleaning row prefixes (`zztest_visibility_*`/`ZZTest Visibility`, `zztest_founder_*`/`ZZTest Founder`) against the real disposable Postgres database, exactly like every other file in this job.
...
- **`backend-legacy-core-journey` fails at any of the ten named steps** — the step name says which file failed; reproduce with the exact `python -m app.tests.<name>` command from "Reproducing the checks locally" above, against your own disposable database.
