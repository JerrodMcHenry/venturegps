...
- **`backend-legacy-core-journey` fails only in "Website URL ingestion -- SSRF hardening"**, on one of the two real-fetch tests (`test_valid_public_https_url_is_fetched`/`test_valid_public_http_url_is_fetched`) — likely a transient outbound-network or `example.com` availability issue, not a code regression; every other case in that file is fully offline.
